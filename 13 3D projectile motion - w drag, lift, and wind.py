...
##wall_width = 0.1
##### Distance to kicker.
##wall_x = 18.288 # Extra point = 30.1752, used to be 18.288.
...
##                             size = vector(wall_width,wall_height,0.1),
##                             opac = 1))
...
min_angle2 = 0.0 # Initial approach angle measured in degrees.
delta_angle2 = 5.0
max_angle2 = 2.0
...
while (angle <= max_angle):
    ...
    while (angle2 <= max_angle2):
        ...
        projectile.area = 3.14159*projectile.radius**2
        ...
        vel_vec = arrow(pos = projectile.pos,
                        color = color.red,
                        axis = projectile.velocity*0.5)

        lift_vec = arrow(pos = projectile.pos,
                        color = color.white,
                        axis = projectile.velocity)

        drag_vec = arrow(pos = projectile.pos,
                        color = color.white,
                        axis = projectile.velocity)

        grav_vec = arrow(pos = projectile.pos,
                        color = color.white,
                        axis = projectile.velocity)
        ...
        while (keep_going):
            ...
            rvhat = (projectile.velocity-wind_velocity) / mag(projectile.velocity-wind_velocity) # Velocity of projectile relative to wind.
            drag_force = 0.5*air_density*drag_coeff*projectile.area*mag(projectile.velocity-wind_velocity)**2*(-rvhat)

            rvhat_perp = cross(vector(0,0,1), rvhat)
            lift_force = 0.5*air_density*lift_coeff*projectile.area*mag(projectile.velocity-wind_velocity)**2*(rvhat_perp)

            force = grav_force + drag_force + lift_force
            ...
            vel_vec.pos = projectile.pos
            vel_vec.axis = projectile.velocity*0.5
            lift_vec.pos = projectile.pos
            lift_vec.axis = lift_force
            drag_vec.pos = projectile.pos
            drag_vec.axis = drag_force
            grav_vec.pos = projectile.pos
            grav_vec.axis = grav_force
